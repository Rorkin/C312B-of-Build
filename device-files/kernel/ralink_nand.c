...
static int nfc_all_reset(void)
{
	...
	ra_outl(NFC_CTRL, ra_inl(NFC_CTRL) | 0x02);
	ra_outl(NFC_CTRL, ra_inl(NFC_CTRL) & ~0x02);
...
int nfc_write_page(struct ra_nand_chip *ra, char *buf, int page, int flags)
{
	...
	oob[ra->badblockpos] = 0xff;
	ra->buffers_page = -1;

	page = page & (CFG_CHIPSIZE-1);
	size = CFG_PAGESIZE + CFG_PAGE_OOBSIZE;
	bus_addr = (page << (CFG_COLUMN_ADDR_CYCLE*8));
...
static int nand_get_device(struct ra_nand_chip *ra, int new_state)
{
	int ret = 0;
...
unsigned int nand_bbt_find_sandbox(struct ra_nand_chip *ra)
{
	...
	while (offs < chipsize) {
		if (nand_block_checkbad(ra, offs))
			break;
		offs += blocksize;
	}
	...
	ra_nand_bbt_set(ra, (unsigned int)offs>>ra->erase_shift, BBT_TAG_RES);
	...
	case MTD_OPS_AUTO_OOB:
		...
		while (size) {
			for(free = ra->oob->oobfree; free->length && size; free++) {
				...
				int bytes = 0;
				if (wlen <= 0) { woffs = -wlen; continue; }
				bytes = min_t(size_t, size, wlen);
				memcpy(buf + free->offset + woffs, oob, bytes);
				woffs = 0; oob += bytes; size -= bytes; retsize += bytes;
			}
			buf += oobsize;
		}
		break;
                             ...
                             int mode, int ooboffs)
{
	...
	case MTD_OPS_AUTO_OOB: {
		struct nand_oobfree *free;
		uint32_t woffs = ooboffs;
		if (ooboffs > ra->oob->oobavail) return -1;
                             ...
                             struct mtd_oob_ops *ops)
{
	...
	if (datalen == 0 && ooblen) {
		...
		if (nfc_enable_chip(ra, addr, 0)) return -EIO;
		page = (int)((to & ((1<<ra->chip_shift)-1)) >> ra->page_shift);
	...
	while(datalen || ooblen) {
		...
		if (oob && ooblen > 0) {
			...
			oob += len; ops->oobretlen += len; ooblen -= len;
		}

		offs = addr & pagemask;
		len = min_t(size_t, datalen, pagesize - offs);
		if (data && len > 0) {
			memcpy(ra->buffers + offs, data, len);
			data += len; datalen -= len; ops->retlen += len;
			ecc_en = FLAG_ECC_EN;
		}
                            ...
                            struct mtd_oob_ops *ops)
{
	...
	while(datalen || ooblen) {
		...
		if (nfc_enable_chip(ra, addr, 1) < 0) return -EIO;
		page = (int)((addr & ((1<<ra->chip_shift)-1)) >> ra->page_shift);
		...
		len = min_t(size_t, datalen, pagesize - offs);
		if (data && len > 0) {
			memcpy(data, ra->buffers + offs, len);
			data += len; datalen -= len; ops->retlen += len;
			if (ret) return ret;
		}
...
{
	if (offs > mtd->size) return -EINVAL;
...
static int one_bit_correction(char *ecc1, char *ecc2, int *bytes, int *bits)
{
	...
	*bytes = 0;
	for (i = 0; i < 9; i++) {
		crumb = (xor >> (2*i)) & 0x3;
		if ((crumb == 0x0) || (crumb == 0x3)) return -1;
		if (crumb == 0x2) *bytes += (1 << i);
	}
	*bits = 0;
	for (i = 0; i < 3; i++) {
		crumb = (xor >> (18 + 2*i)) & 0x3;
		if ((crumb == 0x0) || (crumb == 0x3)) return -1;
		if (crumb == 0x2) *bits += (1 << i);
	}
	return 0;
}
...
#define ALIGNE_16(a) (((unsigned long)(a)+15) & ~15)
	buffers_size = ALIGNE_16((1<<CONFIG_PAGE_SIZE_BIT) + (1<<CONFIG_OOBSIZE_PER_PAGE_BIT));
...
#endif
	ra_outl(NFC_CTRL, ra_inl(NFC_CTRL) | 0x01);
	nfc_all_reset();
